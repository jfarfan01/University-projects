...
Thread::Thread(FileSystemInfo *working_dir, ustl::string name, Thread::TYPE type) :
    ...
    my_terminal_(0), cancel_state_(0), cancel_type_(0), exit_called_flag_(false), working_dir_(working_dir), name_(ustl::move(name))
{
...
UserProcess* Thread::getParentProcess()
{
  return parentProcess_;
}
...
void Thread::setCancelState(size_t state)
{
  ArchThreads::atomic_set(cancel_state_, state);
}

void Thread::setCancelType(size_t type)
{
  ArchThreads::atomic_set(cancel_type_, type);
}

size_t Thread::getCancelState()
{
  return cancel_state_;
}

size_t Thread::getCancelType()
{
  return cancel_type_;
}

void Thread::setExitCalledFlag(bool val)
{
  exit_called_flag_ = val;
}

bool Thread::getExitCalledFlag()
{
  return exit_called_flag_;
}

size_t Thread::tid_counter_ = 0;
